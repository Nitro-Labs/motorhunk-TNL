(() => {
  ...
  function initFilterGroup() {
    ...
    const filterType = this.elms.sidebarContainer.dataset.filterType;
    ...
    filterGroups.forEach((element) => {
      ...
      filterItems.forEach((button) => {
        ...
        button.addEvent("click", (e) => {
          ...
          if (button.hasClass("active")) {
            ...
            if (!enableMultipleValue && filterType != "attribute") {
          ...
          this.getProductList(this.getUrlAjaxRequest(null, "filter", button)).then(() => {
            switch (action) {
              case "add":
      ...
      clearButton?.addEvent("click", (e) => {
        ...
        if (filterType == "attribute") {
          url = clearButton.href;
        } else {
          url = this.getUrlAjaxRequest(null, "filter");
        }
        this.getProductList(url).then(() => {
          clearButton.addClass("d-none");
          clearButton.removeClass("pending");
    ...
    sidebarContainer.querySelector(".js-filter-form")?.addEventListener("submit", (e) => {
      e.preventDefault();
      const formData = new FormData(e.target);
      const query = new URLSearchParams(formData).toString();
      const link = document.createElement("a");
      link.href = `${e.target.action}?${query}`;
      this.getProductList(this.getUrlAjaxRequest(null, "filter", link));
    });
  }

  // app/scripts/common/section/collection-left-sidebar.js
  var CollectionLeftSidebar = {
    onLoad: function() {
      let { container } = this;
    ...
    getUrlAjaxRequest: function(value, action, link) {
      let newURL = "";
      let vendors;
      ...
      switch (action) {
        ...
        case "filter": {
          if (this.filterType == "attribute" && link) {
            newURL = link.tagName == "A" ? link.href : link.querySelector("a").href;
            newURL += "&view=ajax-request";
          } else {
            if (vendors) {
              let paramsSearch = [
    ...
    getProductList: function(url) {
      let { productGridContainer, collectionPaginationContainer, paginationShowing, sidebarContainer } = this.elms;
      return fetch(url, { dataType: "text" }).then((content) => {
        ...
        history.replaceState({}, "", url.replaceAll("?view=ajax-request&", "?").replaceAll("?view=ajax-request", "").replaceAll("&view=ajax-request", ""));
        this.replaceHref(div);
        this.updateFilterForm(div);
    ...
    replaceHref: function(container) {
      if (this.filterType != "attribute")
        return;
      const arrFilterLink = Array.from(this.elms.sidebarContainer.querySelectorAll(".js-filter-link"));
      container.querySelectorAll(".js-filter-link")?.forEach((item) => {
        const currentLink = arrFilterLink.find((i) => i.dataset.filterId == item.dataset.filterId);
        if (currentLink) {
          currentLink.href = item.href;
          currentLink.setAttribute("data-active", item.dataset.active);
          currentLink.closest(".tags-link")?.classList.toggle("active", currentLink.dataset.active == "true");
        }
      });
    },
    updateFilterForm: function(container) {
      container.querySelectorAll(".js-filter-update-content")?.forEach((item) => {
        const target = document.querySelector(`.js-filter-update-content[data-id="${item.dataset.id}"]`);
        if (target) {
  ...
  // app/scripts/collection-left-sidebar-page.js
  (() => {
    register("collection-left-sidebar", CollectionLeftSidebar);
    load("*");
